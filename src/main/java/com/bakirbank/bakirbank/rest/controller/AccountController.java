...
public class AccountController implements AccountControllerApi {
    ...
    @Override
    public ResponseEntity<Boolean> makePayment(MakePaymentRequest makePaymentRequest) {
        return ResponseEntity.status(HttpStatus.OK).body(accountConsumerService.makePayment(makePaymentRequest));
    }
}
